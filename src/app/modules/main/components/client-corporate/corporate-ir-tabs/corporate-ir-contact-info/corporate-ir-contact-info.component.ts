...
@Component({
    ...
    templateUrl: './corporate-ir-contact-info.component.html',
    styleUrl: './corporate-ir-contact-info.component.scss',
})
export class CorporateIrContactInfoComponent implements OnInit {
    ...
    formAddress: FormGroup<AddressForm> = this.fb.group<AddressForm>({
        addressType: this.fb.control<AddressTypeCode | null>(null, {
        ...
        phoneNumber: this.fb.control<string | null>(null, {
            validators: [Validators.pattern(/^\d{8,15}$/)],
        }),
    ...
    private createContactGroup(contactType: ContactType): FormGroup {
        ...
        if (contactType === 4) {
            ...
            g
                ...
                ?.setValidators([
                    Validators.required,
                    Validators.pattern(postalPattern),
                ]);
